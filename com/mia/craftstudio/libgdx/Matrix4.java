...
public class Matrix4 {
	 ...
	 * @param quaternionW The W component of the quaternion that is to be used to set this matrix.
	 * @return This matrix for the purpose of chaining methods together. */
	public Matrix4 set (final float translationX, final float translationY, final float translationZ, final float quaternionX, final float quaternionY,
                        final float quaternionZ, final float quaternionW) {
	 ...
	 * @return This matrix for the purpose of chaining methods together. */
	public Matrix4 set (final float translationX, final float translationY, final float translationZ, final float quaternionX, final float quaternionY,
                        final float quaternionZ, final float quaternionW, final float scaleX, final float scaleY, final float scaleZ) {
		final float xs = quaternionX * 2f, ys = quaternionY * 2f, zs = quaternionZ * 2f;
		final float wx = quaternionW * xs, wy = quaternionW * ys, wz = quaternionW * zs;
		final float xx = quaternionX * xs, xy = quaternionX * ys, xz = quaternionX * zs;
		final float yy = quaternionY * ys, yz = quaternionY * zs, zz = quaternionZ * zs;
	...
	public Matrix4 mul (final Matrix4 matrix) {
		//mul(val, matrix.val);
		this.matrix4_mul(val, matrix.val);
		return this;
	}
	...
	public Matrix4 inv () {
		final float l_det = val[M30] * val[M21] * val[M12] * val[M03] - val[M20] * val[M31] * val[M12] * val[M03] - val[M30] * val[M11]
	...
	public Matrix4 setToProjection (final float near, final float far, final float fovy, final float aspectRatio) {
		idt();
		final float l_fd = (float)(1.0 / Math.tan((fovy * (Math.PI / 180)) / 2.0));
	...
	public Matrix4 setToOrtho (final float left, final float right, final float bottom, final float top, final float near, final float far) {
		...
		this.idt();
		final float x_orth = 2 / (right - left);
		final float y_orth = 2 / (top - bottom);
		final float z_orth = -2 / (far - near);
	 ...
	 * @return This matrix for the purpose of chaining methods together. */
	public Matrix4 setToTranslationAndScaling (final float translationX, final float translationY, final float translationZ, final float scalingX,
                                               final float scalingY, final float scalingZ) {
		idt();
		val[M03] = translationX;
		val[M13] = translationY;
		val[M23] = translationZ;
	...
	public String toString () {
		return String.format("[ %16.10f | %16.10f | %16.10f | %16.10f ]\n[ %16.10f | %16.10f | %16.10f | %16.10f ]\n[ %16.10f | %16.10f | %16.10f | %16.10f ]\n[ %16.10f | %16.10f | %16.10f | %16.10f ]\n",
	...
	private void matrix4_mul(final float[] mata, final float[] matb) {
