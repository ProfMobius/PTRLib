...
public class Quaternion implements Serializable {
	...
	public float[] getEuler(){
		...
		if (pole == 0){
			ro = Math.atan2(2.*(w*z + y*x), 1. - 2.*(x*x + z*z));
			ya = Math.asin(2.*(w*x - z*y));
			pi = Math.atan2(2.*(y*w + x*z), 1. - 2.*(y*y + x*x));
		} else {
			ro = pole * 2f * Math.atan2(y, w);
			ya = pole * Math.PI * 0.5f;
			pi = 0f;
		}

		return new float[]{(float)ya, (float)pi, (float)ro}; 	//We output the rotations around X,Y,Z
	}
	...
	public Quaternion setFromAxisRad (final float x, final float y, final float z, final float radians) {
		...
		if (d == 0f) return idt();
		d = 1f / d;
		final float l_ang = radians < 0 ? MathUtils.PI2 - (-radians % MathUtils.PI2) : radians % MathUtils.PI2;
		final float l_sin = (float)Math.sin(l_ang / 2);
		final float l_cos = (float)Math.cos(l_ang / 2);
	 ...
	 * @param zy z-axis y-coordinate
	 * @param zz z-axis z-coordinate */
	public Quaternion setFromAxes (final boolean normalizeAxes, float xx, float xy, float xz, final float yx, float yy, float yz, float zx,
                                   float zy, float zz) {
		if (normalizeAxes) {
	...
	public Quaternion exp (final float alpha) {
		...
		//Calculate theta
		final float theta = (float)Math.acos(w / norm);
	...
	public float getAxisAngleRad (final Vector3 axis) {
		...
		final double s = Math.sqrt(1 - this.w * this.w); // assuming quaternion normalised then w is less than 1, so term always positive.
